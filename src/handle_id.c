...
/*   Updated: 2020/02/15 22:13:09 by slisandr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_id(t_frm *tmp, va_list argptr, char *c)
{
	...
	else
	{
		...
		if (tmp->plus == 1 && tmp->minus == 1 && tmp->zero == 0 && tmp->sgn != '-')
		{
			flag = 1;
			putchar_and_count('+', tmp);
		}
...
//			printf("%d\n", l);
		...
		//	tmp->w = tmp->w - 1;
		//}
		if (tmp->space == 1 && tmp->minus == 1 && tmp->w != 0 && l != 0 && tmp->sgn != '-' && l != 0)
		{
			putchar_and_count(' ', tmp);
			tmp->w = tmp->w - 1;
		}
		if (tmp->space == 1 && tmp->w != 0 && tmp->sgn != '-' && l != 0 && tmp->zero == 1)
		{
			putchar_and_count(' ', tmp);
			tmp->w = tmp->w - 1;
		}
		...
		if (tmp->w >= 0)
		{
			...
			//		printf("%i %i %i\n", tmp->nb, t, tmp->plus);// check
			if (tmp->zero == 1 && tmp->sgn == '-' && tmp->minus == 0)
				putchar_and_count('-', tmp);
			handle_minus(tmp, c, 0);
			if (tmp->plus == 1 && tmp->sgn != '-' && tmp->zero == 0 && tmp->minus == 0)
				putchar_and_count('+', tmp);
			else if (tmp->zero == 0 && tmp->minus == 0 && tmp->sgn == '-')
				putchar_and_count('-', tmp);
			/* if (l < 0) */
			/* 	l = l * -1; */
		}
